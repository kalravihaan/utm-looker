import { create } from 'zustand';
import { persist } from 'zustand/middleware';
import defaultConfig from '../config/defaultConfig.json';
import { parseFile } from '../utils/dataManager';
import { safeStorage } from './safeStorage';

const generateId = () => `w-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;

const makeDefaultProject = () => ({
  id: 'default',
  name: 'UTM FY 2025-26',
  config: defaultConfig,
  dataSources: [],          // [{ id, name, data[] }]
  activeDataSourceId: null, // null = merge all
});

export const useDashboardStore = create(
  persist(
    (set, get) => ({
      projects: [makeDefaultProject()],
      activeProjectId: 'default',

      // Transient UI state (not persisted)
      activeBrandId: 'overall',
      activeTabIds: {},
      editMode: false,
      selectedWidgetId: null,
      uploading: false,
      uploadError: null,
      sidebarCollapsed: false,

      // ─── Derived ───────────────────────────────────────────────────────────
      getActiveProject: () => {
        const s = get();
        return s.projects.find(p => p.id === s.activeProjectId) || s.projects[0];
      },

      // Returns the merged/selected data array for the active project
      getActiveData: () => {
        const proj = get().getActiveProject();
        if (!proj) return [];
        const sources = proj.dataSources || [];
        if (!sources.length) return [];
        if (proj.activeDataSourceId) {
          const src = sources.find(s => s.id === proj.activeDataSourceId);
          return src?.data || [];
        }
        // Merge all sources
        return sources.flatMap(s => s.data || []);
      },

      // ─── Project management ────────────────────────────────────────────────
      addProject: (name) => {
        const id = `proj-${Date.now()}`;
        const newConfig = { ...defaultConfig, id, title: name };
        set(s => ({
          projects: [...s.projects, { id, name, config: { ...newConfig }, dataSources: [], activeDataSourceId: null }],
          activeProjectId: id,
          activeBrandId: 'overall',
          activeTabIds: {},
        }));
      },

      setActiveProject: (id) => set({ activeProjectId: id, activeBrandId: 'overall', activeTabIds: {}, selectedWidgetId: null }),

      renameProject: (id, name) => set(s => ({
        projects: s.projects.map(p => p.id === id ? { ...p, name } : p),
      })),

      deleteProject: (id) => set(s => {
        const projects = s.projects.filter(p => p.id !== id);
        const fallback = projects.length ? projects : [makeDefaultProject()];
        return {
          projects: fallback,
          activeProjectId: fallback[0].id,
          activeBrandId: 'overall',
        };
      }),

      // ─── Data sources ──────────────────────────────────────────────────────
      uploadData: async (file, sourceName) => {
        set({ uploading: true, uploadError: null });
        try {
          const data = await parseFile(file);
          const dsId = `ds-${Date.now()}`;
          const name = sourceName || file.name.replace(/\.[^.]+$/, '');
          set(s => ({
            projects: s.projects.map(p => {
              if (p.id !== s.activeProjectId) return p;
              // Replace existing source with same name, or add new
              const existing = p.dataSources.findIndex(d => d.name === name);
              const dataSources = existing >= 0
                ? p.dataSources.map((d, i) => i === existing ? { ...d, data, uploadedAt: Date.now() } : d)
                : [...p.dataSources, { id: dsId, name, data, uploadedAt: Date.now() }];
              return { ...p, dataSources };
            }),
            uploading: false,
          }));
        } catch (err) {
          set({ uploading: false, uploadError: err.message });
        }
      },

      deleteDataSource: (dsId) => set(s => ({
        projects: s.projects.map(p =>
          p.id === s.activeProjectId
            ? {
                ...p,
                dataSources: p.dataSources.filter(d => d.id !== dsId),
                activeDataSourceId: p.activeDataSourceId === dsId ? null : p.activeDataSourceId,
              }
            : p
        ),
      })),

      setActiveDataSource: (dsId) => set(s => ({
        projects: s.projects.map(p =>
          p.id === s.activeProjectId ? { ...p, activeDataSourceId: dsId } : p
        ),
      })),

      clearUploadError: () => set({ uploadError: null }),

      // ─── Navigation ────────────────────────────────────────────────────────
      setActiveBrand: (brandId) => set({ activeBrandId: brandId, selectedWidgetId: null }),
      setActiveTab: (brandId, tabId) => set(s => ({
        activeTabIds: { ...s.activeTabIds, [brandId]: tabId },
      })),
      toggleSidebar: () => set(s => ({ sidebarCollapsed: !s.sidebarCollapsed })),

      // ─── Edit mode ─────────────────────────────────────────────────────────
      toggleEditMode: () => set(s => ({ editMode: !s.editMode, selectedWidgetId: null })),
      setSelectedWidget: (id) => set({ selectedWidgetId: id }),

      // ─── Config mutations ──────────────────────────────────────────────────
      updateConfig: (updater) => set(s => ({
        projects: s.projects.map(p =>
          p.id === s.activeProjectId ? { ...p, config: updater(p.config) } : p
        ),
      })),

      // ─── ROS config ────────────────────────────────────────────────────────
      updateRosConfig: (patch) => {
        get().updateConfig(c => ({
          ...c,
          rosConfig: { ...(c.rosConfig || DEFAULT_ROS_CONFIG), ...patch },
        }));
      },

      // ─── Brand mutations ───────────────────────────────────────────────────
      addBrand: () => {
        const id = `brand-${Date.now()}`;
        const newBrand = {
          id, navLabel: 'New Brand', label: 'New Brand',
          color: '#818cf8',
          gradient: 'linear-gradient(135deg,#0f172a,#1e1b4b,#312e81)',
          eyebrow: 'New Dashboard', subtitle: 'Brand Performance',
          filter: null, filterField: 'Brand',
          kpis: [
            { id: `kpi-${generateId()}`, label: 'Sales Qty', field: 'Total Sales Qty', aggregation: 'sum', format: 'compact', color: '#c7d2fe' },
            { id: `kpi-${generateId()}`, label: 'Revenue', field: 'Revenue', aggregation: 'sum', format: 'currency', color: '#c7d2fe' },
          ],
          tabs: [{ id: 'overview', label: 'Overview', widgets: [] }],
        };
        get().updateConfig(c => ({ ...c, brands: [...c.brands, newBrand] }));
        set({ activeBrandId: id });
      },

      deleteBrand: (brandId) => {
        get().updateConfig(c => ({ ...c, brands: c.brands.filter(b => b.id !== brandId) }));
        set(s => ({
          activeBrandId: s.activeBrandId === brandId
            ? (get().getActiveProject()?.config.brands[0]?.id || 'overall')
            : s.activeBrandId,
        }));
      },

      updateBrand: (brandId, patch) => {
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b => b.id === brandId ? { ...b, ...patch } : b),
        }));
      },

      // ─── Tab mutations ─────────────────────────────────────────────────────
      addTab: (brandId) => {
        const tab = { id: `tab-${Date.now()}`, label: 'New Tab', widgets: [] };
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b => b.id === brandId ? { ...b, tabs: [...b.tabs, tab] } : b),
        }));
      },

      deleteTab: (brandId, tabId) => {
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b =>
            b.id === brandId ? { ...b, tabs: b.tabs.filter(t => t.id !== tabId) } : b
          ),
        }));
      },

      renameTab: (brandId, tabId, label) => {
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b =>
            b.id === brandId
              ? { ...b, tabs: b.tabs.map(t => t.id === tabId ? { ...t, label } : t) }
              : b
          ),
        }));
      },

      // ─── Widget mutations ──────────────────────────────────────────────────
      addWidget: (brandId, tabId, widgetType = 'chart') => {
        const id = generateId();
        const widget = widgetType === 'table'
          ? { id, type: 'table', title: 'New Table', span: 2, groupBy: 'Brand', filters: [], columns: [
              { field: 'Brand', label: 'Brand' },
              { field: 'Total Sales Qty', label: 'Sales Qty', aggregation: 'sum', align: 'right', format: 'compact' },
            ]}
          : { id, type: 'chart', title: 'New Chart', chartType: 'bar', span: 1, height: 'md', filters: [],
              dataMapping: { xAxis: 'Month', yAxis: 'Total Sales Qty', groupBy: null },
              aggregation: 'sum', sortBy: 'Month Numbering', color: '#818cf8' };

        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b =>
            b.id === brandId ? {
              ...b, tabs: b.tabs.map(t =>
                t.id === tabId ? { ...t, widgets: [...t.widgets, widget] } : t
              )
            } : b
          ),
        }));
        set({ selectedWidgetId: id });
      },

      updateWidget: (brandId, tabId, widgetId, patch) => {
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b =>
            b.id === brandId ? {
              ...b, tabs: b.tabs.map(t =>
                t.id === tabId
                  ? { ...t, widgets: t.widgets.map(w => w.id === widgetId ? { ...w, ...patch } : w) }
                  : t
              )
            } : b
          ),
        }));
      },

      deleteWidget: (brandId, tabId, widgetId) => {
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b =>
            b.id === brandId ? {
              ...b, tabs: b.tabs.map(t =>
                t.id === tabId ? { ...t, widgets: t.widgets.filter(w => w.id !== widgetId) } : t
              )
            } : b
          ),
        }));
        set({ selectedWidgetId: null });
      },

      reorderWidgets: (brandId, tabId, newWidgets) => {
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b =>
            b.id === brandId
              ? { ...b, tabs: b.tabs.map(t => t.id === tabId ? { ...t, widgets: newWidgets } : t) }
              : b
          ),
        }));
      },

      // ─── KPI mutations ─────────────────────────────────────────────────────
      addKPI: (brandId) => {
        const kpi = { id: `kpi-${generateId()}`, label: 'New KPI', field: 'Total Sales Qty', aggregation: 'sum', format: 'compact', color: '#c7d2fe' };
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b => b.id === brandId ? { ...b, kpis: [...b.kpis, kpi] } : b),
        }));
      },

      updateKPI: (brandId, kpiId, patch) => {
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b =>
            b.id === brandId
              ? { ...b, kpis: b.kpis.map(k => k.id === kpiId ? { ...k, ...patch } : k) }
              : b
          ),
        }));
      },

      deleteKPI: (brandId, kpiId) => {
        get().updateConfig(c => ({
          ...c, brands: c.brands.map(b =>
            b.id === brandId ? { ...b, kpis: b.kpis.filter(k => k.id !== kpiId) } : b
          ),
        }));
      },

      // ─── Calculated fields ─────────────────────────────────────────────────
      addCalculatedField: (field) => {
        get().updateConfig(c => ({
          ...c,
          calculatedFields: [...(c.calculatedFields || []), {
            id: `cf-${Date.now()}`,
            name: field.name || 'New Field',
            formula: field.formula || '0',
            format: field.format || 'number',
            description: field.description || '',
          }],
        }));
      },

      updateCalculatedField: (id, patch) => {
        get().updateConfig(c => ({
          ...c,
          calculatedFields: (c.calculatedFields || []).map(cf => cf.id === id ? { ...cf, ...patch } : cf),
        }));
      },

      deleteCalculatedField: (id) => {
        get().updateConfig(c => ({
          ...c,
          calculatedFields: (c.calculatedFields || []).filter(cf => cf.id !== id),
        }));
      },

      // ─── Import / Export ───────────────────────────────────────────────────
      loadConfigFromFile: async (file) => {
        const text = await file.text();
        try {
          const payload = JSON.parse(text);
          const config = payload.config || payload;
          const dataSources = payload.dataSources || (payload.data ? [{ id: 'ds-0', name: 'Imported', data: payload.data }] : []);
          const id = `proj-${Date.now()}`;
          set(s => ({
            projects: [...s.projects, { id, name: config.title || 'Imported', config, dataSources, activeDataSourceId: null }],
            activeProjectId: id,
          }));
        } catch (e) {
          console.error('Failed to load config:', e);
        }
      },
    }),
    {
      name: 'utm-looker-v2', // bumped version → clears stale v1 data
      storage: safeStorage,
      partialize: (state) => ({
        projects: state.projects.map(p => ({
          id: p.id,
          name: p.name,
          config: p.config,
          dataSources: (p.dataSources || []).map(ds => ({
            id: ds.id,
            name: ds.name,
            uploadedAt: ds.uploadedAt,
            // Cap rows so we don't blow localStorage (~5 MB limit)
            data: (ds.data || []).slice(0, 30000),
          })),
          activeDataSourceId: p.activeDataSourceId,
        })),
        activeProjectId: state.activeProjectId,
        sidebarCollapsed: state.sidebarCollapsed,
      }),
      merge: (persisted, current) => ({
        ...current,
        ...persisted,
        // Migrate old projects that used flat `data` → dataSources
        projects: (persisted.projects || []).map(p => ({
          ...p,
          dataSources: p.dataSources || (p.data?.length ? [{ id: 'ds-0', name: 'Main Data', data: p.data, uploadedAt: Date.now() }] : []),
        })),
        editMode: false,
        selectedWidgetId: null,
        uploading: false,
        uploadError: null,
      }),
    }
  )
);

export const DEFAULT_ROS_CONFIG = {
  useColumn: 'ROS',
  thresholds: { veryFast: 1.5, fast: 0.7, moderate: 0.3 },
};
